        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('.count').on('change', function () {
                    ...
                    $(this).parents('.cart_count').siblings('.item_total').html(getCount(parseInt($(this).parent().parent().siblings('.item_price').html()), $(this).val()) + '元');
                    let all = [];
                    $('input[type="checkbox"]').each((i, item) => {
                        if (i > 0 && $(item).prop('checked')) {
                            all.push($(item).parent().siblings('.item_total').html());
                        }
                    })
                    all = all.map(item => {
                    ...
                    all.length == 0?all=0:all= all.reduce((prev, next) => {
                        return prev + next;
                    })
                    $('.payTotal1').html(all);
                });
